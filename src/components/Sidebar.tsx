...
export const Sidebar = ({
  ...
  return (
    <nav
      className={clsx(
        sidebarListVisibility
          ? 'bg-gray-100 text-current h-full'
          : 'mix-blend-difference text-gray-100 ',
        'fixed z-50 w-full px-4 py-4 border-black border-solid md:relative md:py-8 md:pl-8 md:pr-0 md:bg-none md:text-current md:border-none md:mix-blend-normal md:w-auto select-none',
      )}
    >
...
const TagList = ({ title, content, className }: TagListType) => {
  const listItem = content
    .map(({ uid, tags, first_publication_date, data }: NoteType) => {
      ...
      if (tag == title)
        return (
          <li key={uid}>
            <Link href={'/note/' + uid} passHref>
              <a className={className}>
                {tags[0]
                  ? RichText.asText(data.title)
                  : dateFormatter(first_publication_date)}
              </a>
            </Link>
          </li>
        )
  ...
  if (listItem[0]) {
    return (
      <figure>
        <figcaption className="h-0 transform -translate-y-6">
          <sup>{title}</sup>
        </figcaption>
